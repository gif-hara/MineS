...
namespace MineS
{
	...
	public class FieldCreator
	{
		...
		public CellData[,] Create(CellManager cellManager, DungeonManager dungeonManager, int rowNumber, int culumnNumber)
		{
			var dungeonData = dungeonManager.CurrentData;
			var database = new Database(rowNumber, culumnNumber);

			// 階段を作成.
			this.CreateCellData(cellManager, database, new CreateStairAction());

			// 回復アイテムを作成.
			for(int i = 0, imax = dungeonData.CreateRecoveryItemRange.Random; i < imax; i++)
			{
				this.CreateCellData(cellManager, database, new CreateRecoveryItemAction());
			}

			// 敵を作成.
			for(int i = 0, imax = dungeonData.CreateEnemyRange.Random; i < imax; i++)
			{
				this.CreateCellData(cellManager, database, new CreateEnemyAction());
			}
			...
			{
				this.CreateCellData(cellManager, database, dungeonData.CreateTrap());
			}

			// 鍛冶屋を作成.
			if(dungeonData.CanCreateBlackSmith(dungeonManager.Floor))
			{
				this.CreateCellData(cellManager, database, new CreateBlackSmithAction());
			}

			// 店を作成.
			if(dungeonData.CanCreateShop(dungeonManager.Floor))
			{
				this.CreateCellData(cellManager, database, new CreateShopAction());
			}
		...
		private void CreateCellData(CellManager cellManager, Database database, CellClickActionBase action, int cellIndex)
		{
		...
		private void CreateCellData(CellManager cellManager, Database database, CellClickActionBase action)
		{
